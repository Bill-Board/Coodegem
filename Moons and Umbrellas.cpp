...
{
    /// code is here->
    v99 x,y,k=0;read(x,y);
    string s;read(s);
    for(v99 i=0;i<s.size();i++)
    {
        if(s[i]!='?')
        {
            for(v99 j=i;~j;j--)s[j]=s[i];
            break;
        }
    }
    for(v99 i=1;i<s.size();i++)if(s[i]=='?')s[i]=s[i-1];
    v99 cr_h=0;
    for(v99 i=1;i<s.size();i++)
    {
        if(s[i-1]=='C' and s[i]=='J')cr_h+=x;
        else if(s[i-1]=='J' and s[i]=='C')cr_h+=y;
    }
    cout<<"Case #"<<tc++<<": "<<cr_h<<ses;
