...
{
    v99 cr_h=0;
    for(v99 i=0;i<a.size()-1;i++)
    {
        for(v99 j=i;j<a.size();j++)
        {
            if(a[j]==i+1){cr_h+=j-i+1;reverse( a.begin()+i, a.begin()+j+1);break;}
        }
    }
    return cr_h;
}
multimap<v99,vll>mpp;

void solve()
{
    /// code is here->
    vll a,b;v99 mx=0,mn=LLONG_MAX,n;
    read(n);
    rep1(i,1,n)a.pb(i);
    do
    {
        b.pb(alabu(a));
        mpp.insert({alabu(a),a});
    }while(next_permutation(sob(a)));
    for(auto i:mpp)
    {
        cout<<i.ff<<"\t";
        print(i.ss);
