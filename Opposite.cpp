...
void solve()
{
    /// code is here->
    double n;read(n);
    double x0,y0,xn,yn;read(x0,y0,xn,yn);
    point p0(x0,y0),pn(xn,yn),p1=(p0+pn)/2.0;
    point p2=p1 + (p0-p1)*polar(1.0,2.0*PI/n);
    cout<<fixed<<setprecision(10)<<p2.real()<<whp<<p2.imag()<<ses;
}
